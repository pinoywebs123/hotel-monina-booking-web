    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
          ...
          <li role="presentation" class="active"><a href="{{route('admin_rate')}}">Room rate</a></li>
           <li role="presentation"><a href="{{route('admin_amenities')}}">Amenities</a></li>
         ...
         <div class="panel panel-default">
            <div class="panel-body">
                <div class="col-md-6 col-md-offset-3">
                 ...
                 <form action="{{route('admin_rate_update', ['id'=> $find->id])}}" method="POST" id="updateForm">
                   <div class="form-group {{$errors->has('category_name') ? 'has-error' : ''}}">
                     <label>Category Name:</label>
                     <input type="text" name="category_name" class="form-control" value="{{$find->category_name}}">

                     @if($errors->has('category_name'))
                      <i class="help-block">{{$errors->first('category_name')}}</i>
                     @endif
                   </div>
                   <div class="form-group {{$errors->has('description') ? 'has-error' : ''}}">
                     <label>Description:</label>
                     <textarea name="description" class="form-control" rows="5">{{$find->description}}</textarea>
                   ...
                   <div class="form-group {{$errors->has('price') ? 'has-error' : ''}}">
                     <label>Price:</label>
                     <input type="number" name="price" class="form-control" value="{{$find->price}}">
                     @if($errors->has('price'))
                   ...
                   <div class="form-group {{$errors->has('occupants') ? 'has-error' : ''}}">
                     ...
                     <input type="number" name="occupants" class="form-control" value="{{$find->person}}">
                      @if($errors->has('occupants'))
                      <i class="help-block">{{$errors->first('occupants')}}</i>
                     @endif
                   </div>
                   {{csrf_field()}}
    ...
    <script type="text/javascript">
      $(document).ready(function(){
        $("#updateBtn").click(function(){
            swal({
              title: "Are you sure?",
              text: "Once Updated, Customer will view the updated Room Category instantly!",
              icon: "warning",
              buttons: true,
              dangerMode: true,
            })
            .then((willDelete) => {
              if (willDelete) {
                swal("Room Category has been updated!", {
                  icon: "success", 
                });
